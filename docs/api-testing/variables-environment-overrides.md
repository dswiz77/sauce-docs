...
* Fill the **Vault** with data that is Project-specific, such as domains, protocols, and API keys. This allows you to set the default location within the Compose and then override those amounts. We do not recommend introducing test-specific variables because it would produce an overhead of information that would go unused most of the time.
